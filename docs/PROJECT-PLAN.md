...
| Phase 10 — v2.4.0 candidate from TaskIt 2026-05-11 feedback (12 framework gaps + new feedback-authoring skill) | **CANCELLED 2026-05-17** | — | Superseded by Phase 11 pivot (configure-project-gates meta-skill). ADRs 008-017 retained as catalog citations. B1 (citation re-verification) partially completed inside research-dispatch-3 2026-05-17 (Airflow/Temporal/Argo/Magentic-One primary-source-verified). Other 8 Build cycles converted to catalog entries. |
| Phase 11 — v2.4.0 PIVOT — HARD-GATE catalog + configure-project-gates meta-skill | **IN_PROGRESS (impl complete 2026-05-17; awaiting user validation)** | gate-3 pending | Pivot rationale: F-V40 — feedback log accumulates HARD-GATE proposals but no mechanism converts them to enforced per-project gates. TaskIt 2026-05-15+ batch (F-V7/15/17/29/31/33/34/39 + WORKTREE) overflowed Phase 10 within 4 days. Three-researcher design ratification 2026-05-17: trigger format (4/4 use-case-fit-pass), severity model (5/5 enterprise), phase enforcement (9/9 coordinator-layer). Artifacts: catalog `docs/catalog/hard-gates.md` (42 rows) + `docs/catalog/hard-gates.json` + skill `skills/configure-project-gates/SKILL.md` + hook updates (pre-tool-use + session-start) + framework's own `.framework-state/active-gates.yaml` + CLAUDE.md ## Active Gates section. Dogfood validation: ran configure-project-gates against framework's own repo (meta-project preset); 9 universal + 0 stack-conditional + 18 configurable active. Self-check passed 7/7. See `RELEASE-NOTES.md` v2.4.0 entry. |
...
| Phase 12 — v2.6.0 mechanical floor (sub-agent output verification + write-side scope enforcement + mig-precondition disclosure + mig-dry-apply) | **PLAN_DRAFTED (awaiting CTO ratification + Builder dispatch)** | gate-3 pending | Plan: [docs/plans/v2-6-0-mechanical-floor.md](plans/v2-6-0-mechanical-floor.md). Single-concept scope closing FEEDBACK §1 (sub-agent reliability — 4+ narrative-only DONE recurrences + Architect file-isolation violation) + §2 (mig-precondition — 8 R57/F-22 recurrences in single TaskIt cycle). 5 new gates: `agent-output-file-landed` (universal floor / SubagentStop re-prompt), `subagent-scope-write-enforcement` (universal floor / PreToolUse deny), SubagentStop event-correlation + GC (infra), `mig-precondition-disclosure` Gate A (stack-conditional / cheap pre-dispatch), `mig-dry-apply` Gate B (stack-conditional / Supabase Branch post-write). Research backing: 4 of 6 parallel v2.6 researcher dispatches (R1 hooks, R2 migration safety, R3 multi-agent reconciliation, R6 status tokens) — consolidated at [docs/research/v2-6-design-research-2026-05-27.md](research/v2-6-design-research-2026-05-27.md). All research citations [CITATION-DEGRADED] via WebSearch synthesis (WebFetch domain-denial on Anthropic/arXiv/JS-SPA sites — new FEEDBACK §9.5 finding). 21 PROMPT-only fixes queued for v2.6.x / v2.7 per mechanical-floor filter (FEEDBACK Appendix C). 5 framework-novel items flagged for Path B promotion per ADR-003 (FEEDBACK Appendix D): ACTOR block, Section J table, CLAUDE.md drift composite, `DONE_PENDING_VERIFICATION` token name, filesystem-aware `list_migration_slots`. |
...
| F-V16 | RESEARCH DONE — Build-ready (was MEDIUM, OPEN) | No CI/deploy enforcement — gates live only in the chat session | Honest readiness review, 2026-05-08 — `gate-3-production-check` runs as a discipline checklist invoked inside a CC session. Once the developer leaves the session (commit, PR, deploy), nothing in the framework reasserts the 18 dimensions. A PR could ship that violates D5 (no SLO) or D11 (no audit log) and the framework would not block — only chat-time discipline catches it. **Proposed fix (deferred):** explore a CI shape — `gate-3-production-check.json` machine-readable manifest written during the chat-time invocation; a CI step that fails the build if the manifest is missing or stale; OR a GitHub Action that enforces the checklist at PR time. Cross-references FD-02 (GitHub MCP candidate). **Out of scope for v2.1.1 + v2.2.0;** captures the difference between "framework discipline" and "deployment enforcement" — today only the former exists. **UPDATE 2026-06-01 (Research cycle dogfood):** research + Architect recommendation COMPLETE. Researcher cited 6 enterprise/OSS gate-enforcement implementations (GitHub branch protection, GitLab merge trains, OPA/Conftest, Argo Rollouts, SonarQube, SLSA) → 6/6 consensus: machine-readable verdict + fail-closed default + absent/stale = failure. Architect recommendation: gate-3 emits a `gate-3-<feature>.json` manifest during the chat walk → a pure-bash `scripts/verify-gate-3.sh` CI verifier fails closed on missing/stale/red → bound as a GitHub-native required status check (zero third-party dep; OPA/Conftest opt-in only; commit-drift as the staleness signal). Open concern Q1: the framework can't verify a downstream project actually wired branch protection (inert-if-unwired; intersects FD-02). Artifacts: `docs/research/ci-gate-enforcement-2026-06-01.md`, `docs/architecture/ci-gate-enforcement-2026-06-01.md`. Research phase CLOSED; remaining work = a future Tier 3 Build cycle. | Readiness review, 2026-05-08 |
...
| F-V41 | FIXED — shipped v2.6.1, installed + script-verified 2026-06-01 (was CRITICAL); live agent-dispatch handover still pending | file-scope-intersection + scope parsing truncate paths at the first literal 'n' → garbage in-flight marker → cascading CTO self-deadlock | Direct dogfood reproduction 2026-06-01 (during the F-V16 research cycle, this very session). **Defect 1 (parse):** `hooks/pre-tool-use` `record_agent_start` (and `check_file_scope_intersection`) extract `scope_write`/`scope_read` from the dispatch prompt, but that string still holds escaped (not real) newlines, and the regex character-class meant to stop at a newline instead stops at the literal letter **n**. So every scope path truncates at its first 'n': `production-framework/docs/research/...` was stored as `productio` in `.framework-state/active-agents.jsonl`. **Defect 2 (correlation):** the `stop` event is tagged `subagent_type:"unknown"` (FEEDBACK §1.3), so start↔stop never correlate; the intersection check treats every start within the 30-min window as still in-flight. **Cascade:** `productio` is a substring of EVERY `production-framework/*` path → (a) the read-side `file-scope-intersection` gate false-positive-blocked the legitimate sequential Research→Architect handover; (b) the write-side `subagent-scope-write-enforcement` (§1.2) then applied the stale sub-agent's scope to the MAIN session's own write, blocking the CTO from clearing the marker. Net: any sequential handover — and any main-session write after any dispatch — self-deadlocks for 30 min. **Workaround used:** clear `active-agents.jsonl` via Bash (the §1.2 check doesn't parse Bash file writes — the error's own option (b)). **Proposed fix:** (1) decode escaped newlines before scope extraction (or anchor on real field boundaries, not a newline class); (2) correlate stop↔start by `agent_id` (FEEDBACK §1.3 already specifies this) so completed agents drop out of in-flight; (3) §1.2 must restrict only writes BY an in-flight sub-agent, never the main session. CRITICAL — makes v2.5/v2.6 file-scope enforcement both non-functional (garbage scope) AND actively harmful (deadlocks the orchestrator). Needs its own Debug cycle. **FIX (2026-06-01, superpowers systematic-debugging cycle, source-level):** Defect 1 — added `decode_json_string` + `parse_labeled_field` helpers in `hooks/pre-tool-use`; replaced all 5 truncating newline-class parses (4 in pre-tool-use + output_files in v2-6-helpers.sh) so paths decode escaped newlines and parse line-oriented (no more first-'n' truncation). Defect 2 — added `compute_in_flight_writes` (starts MINUS stops, FIFO by agent type, correlated on the stop event's agent_type); both read-side (`check_file_scope_intersection`) and write-side (`check_write_scope`) now drop completed agents. Defect 3 — `check_write_scope` reads the current call's agent_type from the hook input: main-session writes (no agent context) are no longer scope-restricted; a sub-agent's write is checked only against its OWN type's scope. Tests: `tests/hooks/scope-parsing.test.sh` (3) + `tests/hooks/correlation-and-scope.test.sh` (5, incl. controls proving the gates still deny correctly) — all GREEN; both hooks pass `bash -n`. **Still-open:** (a) Defect 3 assumes a main-session Write's PreToolUse input carries no agent_type while a sub-agent's does — documented CC contract, must be empirically confirmed on PF re-enable; (b) source-level fix — no effect in an installed session until reinstall + Claude Code restart; (c) hooks change is PR-worthy + needs a citation-manifest row per CLAUDE.md. **VERIFY (2026-06-01 rerun):** shipped as v2.6.1 (commit 013447c, pushed to main); user reinstalled → plugin cache holds 2.6.1 carrying the fix (`pre-tool-use` has `decode_json_string` + `compute_in_flight_writes`; the 2.6.0 cache copy has neither). Session flipped to 2.6.1 mid-conversation (the tier-selection skill now resolves from the `2.6.1` cache path; it loaded from `2.6.0` earlier this session). Installed 2.6.1 hooks pass all 8 regression assertions (`tests/hooks/scope-parsing` + `correlation-and-scope`, incl. controls confirming the gates still deny correctly). REMAINING: a live Agent-dispatch Research→Architect handover under 2.6.1 to confirm end-to-end behavior — script-level verified, behavioral run not yet exercised. | Direct dogfood reproduction, 2026-06-01; FIXED + shipped v2.6.1, script-verified 2026-06-01 |
