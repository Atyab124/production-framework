...
Closes two findings surfaced by Taskforge first-real-use (F-V5, F-V6).

**F-V6 — heavy-read-dispatch skill** (the more load-bearing of the two)
- New skill at `skills/heavy-read-dispatch/SKILL.md`. Trigger: task requires reading >3 files of source material to produce a single deliverable (audit, doc generation, codebase survey, pattern catalog, STACK-PATTERNS bootstrap).
- Action: dispatch a `production-framework:researcher` sub-agent BEFORE the first read. Main session never burns context on background reads.
- Orthogonal to `tier-selection`: tier scales execution rigor; heavy-read-dispatch scales context. A Tier 1 doc-generation task that reads 10+ files still goes through a sub-agent.
- HARD-GATE marker: ignoring this returns the framework to v1's CTO-does-everything-in-main-context anti-pattern (the failure mode v2 was forked to escape).
- Citations: Anthropic Agent Tool docs, Anthropic *Effective context engineering*, v2 forking rationale, LangGraph/AutoGen/CrewAI N=3 enterprise consensus, framework's existing output-discipline (≤30-line returns) as the input-discipline counterpart.
- Listed in `using-production-framework` "What Loads Lazily" + cross-referenced from `cto-mode` Checklist Step 1.
- **Empirical evidence:** Taskforge first-real-use, 2026-04-30 — LLM correctly picked Tier 1 per literal rules but read v1 STACK-PATTERNS + v2 template + audit memory + CLAUDE.md in main context to produce a 439-line doc. Rule-compliant by letter; defeated v2's design intent. F-V6 logged + closed in same release.

**F-V5 — CONFIG path indirection for `PROJECT-PLAN.md`**
- `scripts/structural-check.sh` `check_incident_logged()`: now reads `CONFIG.yaml > file_paths.project_plan` first, falls back to convention `docs/PROJECT-PLAN.md`. Projects with an existing plan file at a different path (e.g. `docs/TASKIT-PLAN.md`) point CONFIG at it; the structural check follows.
- `using-production-framework` Shared Context section: declares the indirection rule once — every reference to `docs/PROJECT-PLAN.md` in any skill / agent prompt / template means "the CONFIG-declared path."
- `cto-mode` Checklist Steps 2 + 6: replaced literal `docs/PROJECT-PLAN.md` with the CONFIG-indirection phrasing.
- Origin: Taskforge first-real-use redirected `file_paths.project_plan` to `docs/TASKIT-PLAN.md` because the convention path was an unfilled v1 template; would have left structural-check + skills reading the empty stub instead of the real plan.

No hook contract changes. No agent dispatch shape changes. Minor bump per CLAUDE.md versioning policy ("new skill" = minor).

## v2.0.3 (2026-04-30)

### Trigger-audit instrumentation

Added passive observability for trigger-fidelity audit (Phase E5 step 9 substrate). Every Skill, Agent/Task, and UserPromptSubmit event is now appended as a single JSON line to `.framework-state/trigger-audit.jsonl` in the active project.

- **`hooks/pre-tool-use`** — added `log_invocation()` helper; matches `Skill` and `Agent`/`Task` tool calls and writes `{"timestamp":"...","event":"skill|agent","name":"<skill-or-subagent>"}`. Narrow payload (no Edit/Write/Bash bodies).
- **`hooks/user-prompt-submit`** — extended to also append `{"timestamp":"...","event":"prompt_received","prefix":"<first 200 chars, JSON-escaped>"}`.
- **`hooks/hooks.json`** — `PreToolUse` matcher extended from `Edit|Write|Bash|Skill` to `Edit|Write|Bash|Skill|Agent` so Agent calls reach the script.

No gating semantics changed. The Post-Mortem agent can mine this log for trigger drift (skills that co-fire on the same prompt → possible description overlap; skills that never fire → stale trigger language).

## v2.0.2 (2026-04-30)

### `pre-tool-use` gate-logic fix

The "if either timestamp is empty, allow" branch was unintentionally swallowing the deny case. The intent was to allow when state is uninitialized (no user prompt yet); the actual behavior was to allow whenever `tier_selection_invoked_at` was empty — i.e., always-allow on the very situation the gate exists to catch.

Fix: the early-exit now checks ONLY `last_user_prompt_at`. If a user prompt has fired AND tier-selection hasn't been invoked at or after it, the hook denies as designed.

Verified: Write to `test.md` after a task-shape prompt with no tier-selection invocation now returns `permissionDecision: deny` correctly.

## v2.0.1 (2026-04-30)

### Hook plumbing fixes (post-v2.0.0 verification)

Three defects surfaced during Phase E5 verification on Windows; all fixed.

- **`hooks/user-prompt-submit.sh` → `hooks/user-prompt-submit`** (renamed extensionless). The `run-hook.cmd` polyglot wrapper expects extensionless filenames per its own line 7–9 convention; the `.sh` extension caused the script to be unresolved at runtime. `last_user_prompt_at` is now populated on every user prompt.
- **`hooks/pre-tool-use.sh` → `hooks/pre-tool-use`** (renamed extensionless). Same defect class. The D-A gate is now functional.
- **`hooks/run-hook.cmd`** — added a "hook script not found" diagnostic. When a registered hook's target file is missing, the wrapper now exits non-zero with a clear stderr message instead of swallowing the failure (closes Issue 4 from verification: silent hook failure mode).

No skill or agent body changes. Verification chain (steps 3-4 of `docs/plans/phase-e5-verification-2026-04-30.md`) now reaches the `permissionDecision: deny` JSON path correctly.

## v2.0.0 (2026-04-30)

**First release** of production-framework v2 — a Claude Code plugin forked from Superpowers 5.0.7 (Jesse Vincent, MIT). Adds a CTO-mode orchestrator + 12 specialist sub-agents + 8 execution cycles + production-readiness primitives on top of the SP skill cascade.

### Architecture

- **Forked from Superpowers 5.0.7** — full attribution preserved in `.claude-plugin/plugin.json` `credits` block. SP's 18 skills carry through unchanged.
- **CTO mode + 12 specialist agents** — entry-session adopts CTO orchestrator role; dispatches Architect, Researcher, Builder, QA, Database Engineer, Security/Compliance, SRE/DevOps, Code Reviewer, Debugger, Post-Mortem, Product Manager, UX/Design as standing roles. Each agent's prompt is research-grounded against ≥3 enterprise/OSS frameworks per agent-design research artifacts at `docs/research/agent-design-*.md`.
- **6/6 Opus/Sonnet model matrix** — per SP "least-powerful-model" precedent (`subagent-driven-development/SKILL.md` lines 87–101).

### Skills (36 total = 18 SP-inherited + 18 PF v2)

**Foundational (3):**
- `enterprise-research-first` — N≥3 binding-research-before-design (9/9 enterprise consensus)
- `seven-validation-questions` — pre-execution Tier 2/3 plan gate (Y-Statement format)
- `gate-3-production-check` — 19-dimension production-readiness gate (17 enterprise sources)

**Pass 2 NEW (6 — surfaced by v1-feedback audit Phase 5):**
- `browser-driven-verification` — Playwright as UI/timing-bug verification primitive (5/5 BINDING wait-for-condition)
- `incident-response` — live-fire 5-phase spine (Detect → Triage → Contain → Mitigate → Resolve+Handoff) with rollback-as-first-Contain HARD-GATE
- `parallel-reconciliation` — closing primitive paired with `dispatching-parallel-agents` (5/7 BINDING; verdict-precedence ladder)
- `find-similar-implementations` — 4-step codebase-similarity search before any new helper/component (11/11 BINDING)
- `implementation-decision-log` — append-only decision log between PROJECT-PLAN (phase grain) and STACK-PATTERNS (pattern grain); Microsoft Engineering Playbook 1:1 analog
- `fix-time-hash-check` — advisory pre-fix hash-dedup against PROJECT-PLAN incident table (8/8 BINDING; Rollbar + Datadog corroborate v1 normalization grammar verbatim)

**Wave-3 derived (2):**
- `writing-handover` — rolling-single-doc default cadence (5/9 BINDING; replaces v1 per-wave-immutable; conditional override via `qa_cadence: per-wave`)
- `triage` — bug-shape-prompt routing classifier (Anthropic *Building Effective Agents* routing pattern); pre-condition for `tier-selection` per D-A bundle

**v1 carryforward (2 — un-deferred from v2.1 per ADR-001 G3 amendment + ADR-003):**
- `proposing-patterns` — pattern-registry proposal flow with **dual-path ingest** (Path A: ≥3 distinct incident hashes; Path B: BINDING `enterprise-research-first` + use-case-fit passed)
- `ratify-pattern` — user-gated 6-mechanical-gate ratification + `postpone` 4th disposition (Rust RFC FCP-aligned)

**Originally-planned (5):**
- `regression-scope` — bounded-blast-radius primitive for QA Stage 2
- `rls-aware-migrations` — Postgres migration phase pattern (gh-ost / pgRoll / pt-osc 3/3); FORCE RLS HARD-GATE; client-shape-aware
- `tenant-isolation` — silo/pool/bridge classification + 4-layer checklist (data + cache + log + job-boundary)
- `audit-trail` — 9-field schema + append-only enforcement (NIST AU-2/3/9/11 + ASVS V7 + SOC 2 CC7.2)
- `slo-sli-contracts` — 4 golden signals + multi-window multi-burn-rate alerts + Error Budget Policy

### Hook contract — D-A bundle (NEW; per ADR-002)

**`PreToolUse` hook gates Edit/Write/Bash on 5 disciplines:**

1. `tier-selection` — task-shape prompts must invoke before Edit/Write/Bash
2. Destructive-ops — `rm -rf`, `git reset --hard`, `git push --force`, `DROP TABLE`, `TRUNCATE`, `--no-verify`
3. Phase-break — `Write` to `src/**` requires PROJECT-PLAN current-phase marker
4. Critical-finding-blocks-next-phase — `Write` to `src/**` blocked when CRITICAL findings open
5. Dep-add — `npm/pnpm/yarn/bun add|install` requires Tool-Selection-Chain prefix in prior turn

**Three-tier bypass grammar:** `PF_BYPASS=<rule-id>` per-rule | `PF_BYPASS_ALL=1` + `PF_BYPASS_REASON` session-wide | `.framework-state/PF_GATES_DISABLED` project-level kill switch. All bypasses logged append-only to `.framework-state/bypass-log.jsonl` (Post-Mortem mines for repeat patterns).

**Mechanism:** `permissionDecision: "deny"` JSON form per Anthropic SDK Permissions docs (NOT exit-code-2 — known Claude Code bugs #13744, #36071, #40580 make exit-2 unreliable on Edit/Write matchers).

**Replication of PF v1 Rule #43 STRENGTH (audit Item 41):** PF v1's incident-loop is triple-enforced (`MACHINE(script:structural-check.sh:check_incident_logged) + RULE(agent:deputy) + RULE(agent:post-mortem)`) — described in audit as "the most carefully engineered subsystem in v1." D-A bundle replicates the shape for 5 more rules.

### Architecture Decision Records

- ADR-001 — 7-gap-decisions (G3 amended 2026-04-30: UN-DEFER `proposing-patterns` + `ratify-pattern` from v2.1 to v2.0.x)
- ADR-002 — Hook-Gating Architecture (D-A bundle scope; per-rule recommendation table; bypass grammar)
- ADR-003 — Broadened Pattern Ingest (Path A + Path B dual-path per `proposing-patterns` Wave 2 R-1)

### Phase 6 research (Pass 2 grounding) — 14 artifacts

All under `docs/research/` — full sources inventory + verbatim citations + K/N consensus + R-numbered recommendations per artifact. Total ~3500 lines of cited research feeding the 18 PF v2 skills + 9 amendments + 3 ADRs.

### v1 feedback audit (Phase 5)

`docs/audits/v1-feedback-vs-v2-2026-04-30.md` — 36 items audited; 11 clusters; 6 architectural decisions surfaced (D-A through D-F). Closes 35 of 36 items in v2.0; Item 33 (truncated, likely `stop-debug-scan`) deferred pending user clarification.

### Compatibility

- Claude Code (primary target)
- Cursor (inherits SP cursor support)
- Codex (inherits SP codex support)
- OpenCode (inherits SP opencode support)
- GitHub Copilot CLI (inherits SP copilot support)

### Known issues

- D19 console-errors-clean dimension has no deterministic grep — Playwright execution is sole enforcement. Documented in skill body.
- D-A hook bundle requires Claude Code with `permissionDecision: "deny"` JSON support (verified working on CC 2.1.x; older versions may fall back to permissive — document `min-cc-version` per project's Stack Config).

---

# Superpowers Release Notes (inherited)
